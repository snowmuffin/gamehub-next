...
const Sidebar = ({ local_variable, ThemeChanger }: any) => {
  ...
  useEffect(() => {
    ...
    // Replace Wiki menu item with dynamic version
    const updatedMenuItems = MENUITEMS.map(item => {
      if (item.title === "Wiki") {
        return generateWikiMenuItem(wikiCategories, language);
      }
      return item;
    });
    
    console.log("Final menu items:", updatedMenuItems);
  ...
  function Onhover() {
    const theme = store.getState().local_variable;
    if (
      (theme.dataToggled == "icon-overlay-close" || theme.dataToggled == "detached-close") &&
  ...
  function slideRight(): void {
    ...
    if (menuNav && mainContainer1) {
      ...
      if (menuNav.scrollWidth > mainContainer1.offsetWidth) {
        if (!(local_variable.dataVerticalStyle.dir === "rtl")) {
  ...
  useEffect(() => {
    ...
    let currentPath: any = pathname.endsWith("/") ? pathname.slice(0, -1) : pathname;
